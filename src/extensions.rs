...
use structures::{Cookie, NamedGroup, NamedGroupList, SignatureScheme, SignatureSchemeList,
                 ...
                 PreSharedKeyExtension, KeyShare, KeyShareEntry, PskIdentity, PskBinderEntry,
                 EarlyDataIndication, EarlyDataIndicationOptions, Empty};
...
impl Extension {
    ...
    pub fn parse_signature_algorithms<'a>(iter: &mut Iter<'a, u8>) -> Result<Extension, TLSError> {
        ...
        Ok(Extension::SignatureAlgorithms(SignatureSchemeList {
                                              supported_signature_algorithms: ret,
                                          }))
    }
    ...
    fn parse_keyshare_entry<'a>(iter: &mut Iter<'a, u8>) -> Result<KeyShareEntry, TLSError> {
        ...
        let namedgroup: NamedGroup = match ((*first.unwrap() as u16) << 8) |
                                           (*second.unwrap() as u16) {
            0x0017 => NamedGroup::secp256r1,
            0x0018 => NamedGroup::secp384r1,
            0x0019 => NamedGroup::secp521r1,
            0x001d => NamedGroup::x25519,
    ...
    fn parse_psk_identities<'a>(iter: &mut Iter<'a, u8>) -> Result<Vec<PskIdentity>, TLSError> {
        ...
        for _ in 0..length {
			...
			*/
            ...
            let obfuscated_age = ((*b1 as u32) << 24) | ((*b2 as u32) << 16) | ((*b3 as u32) << 8) |
                                 (*b4 as u32);

            ret.push(PskIdentity {
    ...
    pub fn parse_preshared_key<'a>(iter: &mut Iter<'a, u8>) -> Result<Extension, TLSError> {
		...
		*/
        ...
        Ok(Extension::PreSharedKey(PreSharedKeyExtension {
                                       identities: pski_list,
                                       binders: pskb_list,
                                   }))
    }

    pub fn parse_earlydata<'a>(_: &mut Iter<'a, u8>) -> Result<Extension, TLSError> {
        /*
			Technically, the format of this extension depends on whether we are parsing
		...
		*/
        ...
        Ok(Extension::EarlyData(EarlyDataIndication {
                                    value: EarlyDataIndicationOptions::ClientHello(Empty {}),
                                }))
    }
    ...
    pub fn parse_psk_key_exchange_modes<'a>(iter: &mut Iter<'a, u8>)
                                            -> Result<Extension, TLSError> {
        let first = iter.next().unwrap();
    ...
    pub fn parse_oldfilters<'a>(_: &mut Iter<'a, u8>) -> Result<Extension, TLSError> {
