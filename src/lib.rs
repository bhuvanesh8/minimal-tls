...
use structures::{Random, ClientHello, CipherSuite, Extension, ContentType, HandshakeMessage,
...
pub fn tls_configure(cert_path: &str, key_path: &str) -> Result<TLS_config, TLSError> {
    ...
    try!(certfile
             .read_to_string(&mut filebuf)
             .or(Err(TLSError::InvalidCertificate)));
    ...
    try!(keyfile
             .read_to_string(&mut filebuf)
             .or(Err(TLSError::InvalidPrivateKey)));

    // Generate PKey object
    let eckey = try!(EcKey::private_key_from_pem(filebuf.as_bytes())
                         .or(Err(TLSError::InvalidPrivateKeyFile)));
...
pub fn tls_init<'a, R: Read, W: Write>(read: &'a mut R,
                                       write: &'a mut W)
                                       -> Result<TLS_session<'a>, TLSError> {
    ...
    let mut ret = TLS_session {
        ...
        ctypecache: ContentType::InvalidReserved,
        recordcache: vec![],
...
impl<'a> TLS_session<'a> {
    ...
    */
    fn read(&mut self, dest: &mut [u8]) -> Result<usize, TLSError> {
        ...
        let len = dest.len();
        dest.clone_from_slice(self.recordcache
                                  .drain(0..len)
                                  .collect::<Vec<u8>>()
                                  .as_slice());
    ...
    fn write_encrypted(&mut self, src: &[u8]) -> Result<usize, TLSError> {
        let ciphertext = try!(self.create_tlsciphertext(ContentType::ApplicationData,
                                                        src.to_vec()));
    ...
    fn fill_recordcache_encrypted(&mut self) -> Result<(), TLSError> {
        ...
        // Decrypt the data
        let mut decrypted = try!(crypto::aead_decrypt(&self.aead_read_key,
                                                      &self.aead_read_nonce,
                                                      &tlsciphertext.encrypted_record));

        // Remove padding
        let end = match (0..decrypted.len())
                  .rev()
                  .find(|&x| decrypted[x] != 0x00) {
            Some(x) => x,
        ...
        self.aead_read_nonce = try!(crypto::generate_nonce(self.read_sequence_number,
                                                           &self.aead_read_iv));

        Ok(())
    }

    fn create_tlsplaintext(&mut self,
                           contenttype: ContentType,
                           data: Vec<u8>)
                           -> Result<TLSPlaintext, TLSError> {
        Ok(TLSPlaintext {
               ctype: contenttype,
    ...
    fn create_tlsciphertext(&mut self,
                            contenttype: ContentType,
                            data: Vec<u8>)
                            -> Result<TLSCiphertext, TLSError> {
        let innerplaintext = TLSInnerPlaintext {
            content: data,
            ctype: contenttype,
            zeros: vec![],
        };
        ...
        let encrypted_record = try!(crypto::aead_encrypt(&self.aead_write_key,
                                                         &self.aead_write_nonce,
                                                         &innerplaintext.as_bytes()));

        Ok(TLSCiphertext {
               opaque_type: ContentType::ApplicationData,
               legacy_record_version: 0x0301,
               length: encrypted_record.len() as u16,
    ...
    fn send_tlsciphertext(&mut self, tlsciphertext: TLSCiphertext) -> Result<(), TLSError> {
        ...
        self.aead_write_nonce = try!(crypto::generate_nonce(self.write_sequence_number,
                                                            &self.aead_write_iv));
    ...
    fn get_next_tlsciphertext(&mut self) -> Result<TLSCiphertext, TLSError> {
        ...
        try!(self.reader
                 .read_exact(encrypted_record.as_mut_slice())
                 .or(Err(TLSError::ReadError)));
    ...
    fn read_clienthello(&mut self) -> Result<ClientHello, TLSError> {
        ...
        let buffer = HandshakeBytes {
            ...
            .as_bytes();
        unsafe {
            crypto::crypto_hash_sha256_update(stateptr, buffer.as_ptr(), buffer.len() as u64)
        };
    ...
    fn negotiate_ciphersuite(&mut self,
                    ...
                    if let KeyShare::ClientHello(ref ks) = *kso {
                        ...
                        match (*ks).iter().find(|&x| x.group == NamedGroup::x25519) {
                            Some(x) => {
                                ...
                                let (shared_key, server_pub) =
                                    try!(crypto::x25519_key_exchange(&x.key_exchange));
        ...
        if !messagequeue.is_empty() {
            ...
            for x in &messagequeue {
                ...
                if data.len() + ret.len() > (16384 - 4) {
                    ...
                    let tlsplaintext = try!(self.create_tlsplaintext(ContentType::Handshake,
                                                                     data.drain(..).collect()));
    ...
    fn send_encrypted_message(&mut self,
                              encryptedqueue: Vec<HandshakeMessage>)
                              -> Result<(), TLSError> {
        if !encryptedqueue.is_empty() {
            ...
            for x in &encryptedqueue {
                ...
                if data.len() + ret.len() > 16384 {
                    ...
                    // Flush the existing messages, then continue
                    let tlsciphertext = try!(self.create_tlsciphertext(ContentType::Handshake,
                                                                       data.drain(..).collect()));
        ...
        let result = match self.state {
            ...
            TLSState::Negotiated => {
                ...
                */
                ...
                self.handshake_secret = try!(crypto::generate_handshake_secret(&self.shared_key,
                                                                               &derivedsecret));

                let (server_hts, client_hts) = try!(crypto::generate_hts(&self.handshake_secret,
                                                                         &self.th_state));
                ...
                try!(self.add_to_thstate(HandshakeType::EncryptedExtensions,
                                         &encrypted_extensions));

                encryptedqueue.push(encrypted_extensions);

                // Send Certificate
                let cert_message_list = config
                    .certificates
                    .iter()
                    .map(|x| {
                             CertificateEntry {
                                 cert_data: x.contents.clone(),
                                 extensions: vec![],
                             }
                         })
                    .collect();
                let cert_message = HandshakeMessage::Certificate(Certificate {
                ...
                let signature = try!(crypto::generate_cert_signature(&config.private_key,
                                                                     &self.th_state));
            ...
            */
            TLSState::WaitEndOfEarlyData => Err(TLSError::InvalidState),
            ...
            */
            TLSState::WaitCert => Err(TLSError::InvalidState),
            TLSState::WaitCertificateVerify => Err(TLSError::InvalidState),
            ...
            TLSState::WaitFinished => {
                ...
                // Verify the message
                try!(crypto::verify_finished(&self.th_state,
                                             &self.client_hts,
                                             &finished_message.verify_data));

                // Generate application traffic secret
                let derivedsecret = try!(crypto::generate_derived_secret(&self.handshake_secret));
                let (server_traffic_secret, client_traffic_secret) =
                    try!(crypto::generate_atf(&derivedsecret, &self.th_state));
                ...
                let (aead_key, aead_iv) =
                    try!(crypto::generate_traffic_keyring(&self.server_traffic_secret));
                self.aead_write_key = aead_key;
                self.aead_write_iv = aead_iv;

                let (aead_key, aead_iv) =
                    try!(crypto::generate_traffic_keyring(&self.client_traffic_secret));
                self.aead_read_key = aead_key;
                self.aead_read_iv = aead_iv;
